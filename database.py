...
def _gstin(state_code: str, pan: str) -> str:
    """Build a GSTIN with a valid check character (format-valid, not a real registration)."""
    alphabet = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ"
    body = f"{state_code}{pan}1Z"
    total = 0
    for index, character in enumerate(body):
        product = alphabet.index(character) * (1 if index % 2 == 0 else 2)
        total += product // 36 + product % 36
    return body + alphabet[(36 - total % 36) % 36]


def _demo_startup(business_id, password, name, pan, cin, sector, domain, employees, stage, address, pincode, website, representative, title, incorporated, business_type="Private Limited"):
    email_domain = website.split("//", 1)[1]
    digits = str(10000 + sum(ord(character) * (index + 1) for index, character in enumerate(business_id)) % 90000)
    return {
        ...
        "cin": cin,
        "ownership_details": "Indian-owned, DPIIT-recognised startup (demo record)",
        ...
        "company_email": f"hello@{email_domain}",
        "company_phone": f"+91 90000 {digits}",
        "website": website,
        "pincode": pincode,
        "representative_name": representative,
        "representative_title": title,
        "representative_email": f"{representative.split()[0].lower()}@{email_domain}",
        "representative_phone": f"+91 91000 {digits}",
        "gstin": _gstin("27", pan),
        "gst_state": "Maharashtra",
        "gst_details": "Regular GST registration (demo record)",
    }


# Demo accounts for local testing. All data is fictional; websites use the reserved
# .example domain. Set STARTUP_CONNECT_SEED_DEMO_DATA=false to stop seeding them.
DEMO_STARTUPS = [
    {
    ...
    _demo_startup("netrascan", "NetraScan@2026", "NetraScan Health", "AAHCN4821K", "U85100MH2021PTC361204",
                  "Health-tech", "Smartphone retinal imaging with AI screening for diabetic retinopathy", 31, "Early revenue",
                  "Baner, Pune, Maharashtra", "411045", "https://netrascan.example", "Dr. Meera Deshpande", "Founder & CEO", "2021-03-09"),
    _demo_startup("jalrakshak", "JalRakshak@2026", "JalRakshak Technologies", "AAHCJ7314M", "U33100MH2020PTC349871",
                  "Climate-tech", "Low-cost IoT sensors for real-time rural water quality monitoring", 18, "Growth",
                  "Satpur MIDC, Nashik, Maharashtra", "422007", "https://jalrakshak.example", "Rohan Patil", "Co-founder & CTO", "2020-08-21"),
    _demo_startup("sahajseva", "SahajSeva@2026", "SahajSeva Digital", "AAHCS2209P", "U72200MH2022PTC382519",
                  "Gov-tech", "Accessible multilingual kiosks and assisted-service software for citizen centres", 12, "Early revenue",
                  "Andheri East, Mumbai, Maharashtra", "400069", "https://sahajseva.example", "Farah Shaikh", "Chief Executive Officer", "2022-01-17"),
    _demo_startup("gatiflow", "GatiFlow@2026", "GatiFlow Mobility", "AAHCG5567Q", "U74999MH2019PTC327740",
                  "Mobility", "Adaptive traffic signal control using edge video analytics", 46, "Scale-up",
                  "Hinjawadi Phase 1, Pune, Maharashtra", "411057", "https://gatiflow.example", "Aditya Joshi", "Managing Director", "2019-11-02"),
    _demo_startup("poshantrack", "PoshanTrack@2026", "PoshanTrack Labs", "AAHCP8834R", "U85300MH2023PTC401126",
                  "Health-tech", "Mid-day meal and child growth tracking for schools and anganwadis", 9, "Idea / pre-incorporation",
                  "Dharampeth, Nagpur, Maharashtra", "440010", "https://poshantrack.example", "Sneha Wankhede", "Founder", "2023-06-12"),
    _demo_startup("bhashamitra", "BhashaMitra@2026", "BhashaMitra AI", "AAHCB3390S", "U72900MH2021PTC368053",
                  "AI / ML", "Marathi and Hindi voice assistants for public service helplines", 27, "Growth",
                  "Kharadi, Pune, Maharashtra", "411014", "https://bhashamitra.example", "Omkar Gokhale", "Co-founder & CEO", "2021-09-28"),
    _demo_startup("kachrasetu", "KachraSetu@2026", "KachraSetu CleanTech", "AAHCK6178T", "U90000MH2020PTC344512",
                  "Smart City", "Computer-vision waste monitoring and collection-route optimisation for municipalities", 22, "Early revenue",
                  "Wagle Estate, Thane, Maharashtra", "400604", "https://kachrasetu.example", "Kavya Iyer", "Chief Executive Officer", "2020-04-15"),
]
SAMPLE_STARTUP_DATA = DEMO_STARTUPS[0]
...
def demo_seeding_enabled() -> bool:
    return os.getenv("STARTUP_CONNECT_SEED_DEMO_DATA", "true").strip().lower() not in {"false", "0", "no", "off"}


def _seed_demo_startups(connection, cursor, cipher: Fernet) -> int:
    """Insert any missing demo startups and commit. Returns how many were added."""
...
def seed_demo_startups() -> int:
    """Create the demo startup accounts in MySQL if they are missing."""
    cipher = _cipher()
    connection = _open_connection()
    cursor = connection.cursor()
    try:
        cursor.execute(SCHEMA_SQL)
        return _seed_demo_startups(connection, cursor, cipher)
    finally:
        cursor.close()
        connection.close()
...
def _prepare_auth_tables(connection, cursor, cipher: Fernet) -> None:
    ...
    _seed_demo_startups(connection, cursor, cipher)
...
def confirm_challenge_final_selection(
    ...
    try:
        ...
        cursor.execute(
            "SELECT selection_id FROM challenge_final_selections WHERE challenge_id = %s",
            (challenge_id,),
        )
        if cursor.fetchone():
            connection.rollback()
...
def list_startup_directory() -> list[dict[str, Any]]:
    ...
    try:
        cursor.execute(SCHEMA_SQL)
        _seed_demo_startups(connection, cursor, _cipher())
        cursor.execute(
